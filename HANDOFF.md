...
# PHẦN E — BLOG SEO/GEO (bổ sung 07/2026)

Folder `bai-viet/` giao kèm: **20 bài markdown + 20 cover PNG + INDEX.md** (đọc INDEX.md để tích hợp).
- Route: `/bai-viet` (danh sách, filter 4 chủ đề) + `/bai-viet/[slug]` (SSG bằng gray-matter + remark — KHÔNG client-fetch).
- Mỗi bài: frontmatter đầy đủ (title/description/keywords/category/cover/date/updated/faq[]) → generateMetadata + JSON-LD Article + FAQPage (nguyên văn khớp phần FAQ hiển thị).
- Sitemap: thêm 20 URL /bai-viet/<slug>, lastmod = updated. Nav + footer đã có link "Bài viết" trong design reference.
- Design tham chiếu: `Bài viết.dc.html` (danh sách) + `Bài viết chi tiết.dc.html` (trang bài — breadcrumb, H1 Prata, body 760px, FAQ card, CTA #B05F70).
- Giọng văn bài mới: xưng "Hà Phương", gọi "mẹ", mở đầu bằng "**Trả lời nhanh:**", giá dạng 60.000đ, bài sức khoẻ có dòng miễn trừ y khoa.
